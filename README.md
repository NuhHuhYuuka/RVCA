<div align="center">
  ...
  <h1>RVCA — Realtime Video Call Application</h1>

  <p>
    <strong>Ứng dụng chat / voice / video P2P phân tán</strong><br/>
    Viết bằng C# / .NET 10 · WinForms · Tích hợp AI Bot Uiti-chan
  </p>
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/UI-WinForms-blueviolet?style=flat-square" alt="WinForms"/>
    <img src="https://img.shields.io/badge/E2EE-AES--256--GCM-success?style=flat-square&logo=letsencrypt" alt="E2EE"/>
    <img src="https://img.shields.io/badge/Audio-Opus%20%2F%20NAudio-orange?style=flat-square" alt="Opus"/>
    <img src="https://img.shields.io/badge/AI%20Bot-OpenRouter%20%2B%20Whisper%20%2B%20VoiceVox-ff69b4?style=flat-square" alt="AI Bot"/>
  </p>
</div>
...
| Nhóm | Tính năng |
|---|---|
| **Chat** | Tin nhắn 1-1 (E2EE: ECDH ephemeral + AES-256-GCM), chat nhóm, gửi file/ảnh (SHA-256 verify) |
| **Voice / Video** | Gọi 1-1 (Opus + JPEG/UDP) · **Gọi nhóm full-mesh** (multi-peer) · **Share màn hình** (presenter mode 960×720) |
| **AI Bot Uiti-chan** | Chat text · **Voice call** (Whisper STT + LLM + VoiceVox TTS + echo suppression half-duplex) · Bot trong nhóm chat |
| **Bảo mật** | E2EE 1-1, AES-CBC cho bot, PBKDF2 cho password, OTP qua Gmail |
| **Network** | Auto-detect Tailscale IP, NAT remap, P2P direct TCP + relay fallback, hỗ trợ 3 mode demo (LAN / Tailscale / Azure VPS) |
...
### Gọi 1-1 (P2P)
- Audio (Opus 48kHz/24kbps) và video (JPEG 320×240/UDP) **P2P trực tiếp giữa hai client** — không đi qua server.
- Cả hai client phải mở được port UDP inbound để nhận stream từ peer.
- Cùng LAN / WiFi: hoạt động ngay.
- Khác mạng: dùng Tailscale (VPN mesh) — RVCA tự động chọn IP Tailscale khi đăng nhập.

### Gọi nhóm (Full-mesh)
- Mỗi thành viên gửi UDP tới **tất cả** peer khác (N×(N-1) connections).
- Signaling qua TCP/relay: `GROUP_VOICE_JOIN` / `GROUP_VOICE_REPLY` / `GROUP_VOICE_LEAVE`.
- Pending buffer (joins + replies) + retry 3s/6s → robust với race condition.
- Video group có **share màn hình**: bấm `🖥️ Chia sẻ MH` → frame 960×720 q60 thay webcam → peer nhận tile tự phóng to (presenter mode).

### Bot voice call (UitiChan)
- Bấm gọi UitiChan → bot nhận voice qua UDP → STT (Groq Whisper) → LLM (OpenRouter) → TTS (VoiceVox JP) → gửi audio Opus về.
- **Echo suppression half-duplex**: khi bot đang nói, mic client tự suppress (600ms) → tránh feedback loop loa→mic→bot.
- Set 2 env vars trước khi chạy bot: `OPENROUTER_API_KEY`, `GROQ_API_KEY`.
...
| Bot voice call: nghe được nhưng bot không phản hồi | Bot đang ở WAN IP sai → restart bot, check log có dòng `[IP] Tailscale CGNAT: 100.x.x.x` không |
| Group voice/video: thấy peer nhưng không nghe/thấy | Restart client; check log status bar có lỗi `Lỗi voice: ...` không |
| Share màn hình bị mờ | Source 960×720 quality 60 ổn định; nếu vẫn mờ kiểm tra MTU mạng Tailscale (frame >63KB sẽ drop) |
| Bot voice loop echo (bot trả lời chính nó) | Echo suppression đã tự bật cho bot call; nếu vẫn loop → tăng `EchoSuppressMs` trong `VoiceCallService.cs` |
